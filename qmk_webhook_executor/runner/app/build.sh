...
# Info helper
st_info() {
    echo -e "\e[38;5;44mInfo:\e[38;5;37m $@\e[0m" >&2
}
...
collect_modified_keymaps() {
    ...
    done < <(pcmd git status -s | pcmd awk '/^M/ {print $2}' | pcmd sed -e 's@^keyboards/@@g' -e 's@/keymaps.*@@g' | sort | uniq)
}
...
if [[ $retcode -eq 0 ]] && [[ ! -e /__w/failed.$intended_sha1 ]] ; then
    ...
    collect_modified_keymaps
    # If there were no failures, attempt to build any keyboards that were added
...
<h4>&gt;&gt;&gt; Merge to: $target_branch</h4>
<h4>&gt;&gt;&gt; Author: $pr_author</h4>
<h4>&gt;&gt;&gt; Title: $pr_title</h4>
EOF
