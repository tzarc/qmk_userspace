...
//===========================================================================
// Modified by Nick Brassel (@tzarc) for use within QMK:
// - Removed C++ API
// - Removed Arduino.h include
// - Added stdlib.h, progmem.h
//===========================================================================
#include "gif.h"
...
static void GIFMakePels(GIFIMAGE *pPage, unsigned int code, void *pExtData);
static int DecodeLZW(GIFIMAGE *pImage, int iOptions, void *pExtData);
...
// C API
int GIF_openRAM(GIFIMAGE *pGIF, const uint8_t *pData, int iDataSize, GIF_DRAW_CALLBACK *pfnDraw)
{
...
// -1 = error
//
int GIF_playFrame(GIFIMAGE *pGIF, int *delayMilliseconds, void *pExtData)
{
int rc;
...
static void GIFMakePels(GIFIMAGE *pPage, unsigned int code, void *pExtData)
{
    ...
    while (iPixCount && pPage->iYCount > 0)
    {
        ...
        else  /* Pixels cross into next line */
        {
            ...
            (*pPage->pfnDraw)(&gd, pPage, pExtData); // callback to handle this line
            pPage->iYCount--;
            buf = pPage->ucLineBuf;
...
init_codetable:
    ...
    oldcode = code;
    GIFMakePels(pImage, code, pExtData); // first code is output as the first pixel
    // Main decode loop
    while (code != eoi && pImage->iYCount > 0) // && y < pImage->iHeight+1) /* Loop through all lines of the image (or strip) */
    {
        ...
        if (code != eoi)
        {
            ...
            GIFMakePels(pImage, code, pExtData);
            oldcode = code;
        }
