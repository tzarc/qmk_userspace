...
void usart_init(void) {
    ...
    palSetPadMode(PAL_PORT(SERIAL_USART_TX_PIN), PAL_PAD(SERIAL_USART_TX_PIN), PAL_MODE_STM32_ALTERNATE_OPENDRAIN);
    palSetPadMode(PAL_PORT(SERIAL_USART_RX_PIN), PAL_PAD(SERIAL_USART_RX_PIN), PAL_MODE_STM32_ALTERNATE_OPENDRAIN);
#else
    palSetPadMode(PAL_PORT(SERIAL_USART_TX_PIN), PAL_PAD(SERIAL_USART_TX_PIN), PAL_MODE_ALTERNATE(SERIAL_USART_TX_PAL_MODE) | PAL_STM32_OTYPE_OPENDRAIN);
    palSetPadMode(PAL_PORT(SERIAL_USART_RX_PIN), PAL_PAD(SERIAL_USART_RX_PIN), PAL_MODE_ALTERNATE(SERIAL_USART_RX_PAL_MODE) | PAL_STM32_OTYPE_OPENDRAIN);
#endif
}
...
void soft_serial_initiator_init(SSTD_t* sstd_table, int sstd_table_size) {
    ...
    sdcfg.cr2 &= ~USART_CR2_SWAP; // master has non-swapped TX/RX pins
    usart_init();
}
...
void soft_serial_target_init(SSTD_t* sstd_table, int sstd_table_size) {
    ...
    sdcfg.cr2 |= USART_CR2_SWAP; // slave has swapped TX/RX pins
    usart_slave_init();
}
