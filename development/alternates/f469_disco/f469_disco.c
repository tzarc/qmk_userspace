...
void SDRAM_Init(void) {
    ...
    static const struct {
        stm32_gpio_t *port;
        uint8_t       pad;
    } sdram_pins[] = {// Address lines A0-A11
                      {GPIOF, 0},
                      {GPIOF, 1},
                      {GPIOF, 2},
                      {GPIOF, 3},
    ...
    // SDCR[0] and SDCR[1] configuration
    ...
    FMC_Bank5_6->SDCMR = (1 << FMC_SDCMR_CTB1_Pos) | // Target Bank 1
    ...
    FMC_Bank5_6->SDCMR = (7 << FMC_SDCMR_NRFS_Pos) | // Number of auto-refresh: 8
                         (1 << FMC_SDCMR_CTB1_Pos) | // Target Bank 1
    ...
    // Mode Register: Burst Length=1, Burst Type=Sequential, CAS=3, Op Mode=Standard, Write Burst=Single
    uint32_t mode_register = (0 << 9) | // Write Burst Mode: Single location access
                             (0 << 7) | // Operating Mode: Standard
                             (3 << 4) | // CAS Latency: 3
                             (0 << 3) | // Burst Type: Sequential
                             (0 << 0);  // Burst Length: 1

    FMC_Bank5_6->SDCMR = (mode_register << FMC_SDCMR_MRD_Pos) | // Mode register value
                         (1 << FMC_SDCMR_CTB1_Pos) |            // Target Bank 1
    ...
    // Register value is shifted left by 1
