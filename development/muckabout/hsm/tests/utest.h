...
#        define UTEST_INITIALIZER_BEGIN_DISABLE_WARNINGS _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wglobal-constructors\"")
...
#    if defined(__clang__)
#        define UTEST_INITIALIZER_BEGIN_DISABLE_WARNINGS _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wmissing-variable-declarations\"")
...
#    define UTEST_INITIALIZER(f)                                                                                                                        \
        ...
        __pragma(comment(linker, "/include:" UTEST_SYMBOL_PREFIX #f "_")) UTEST_C_FUNC __declspec(allocate(".CRT$XCU")) void(__cdecl * f##_)(void) = f; \
...
*/
#    if defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901L) || defined(__cplusplus) && (__cplusplus >= 201103L) || (defined(__MINGW32__) || defined(__MINGW64__))
...
#    endif
#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 201112L) && !(defined(__MINGW32__) || defined(__MINGW64__)) || defined(__TINYC__)
#    define utest_type_printer(val) UTEST_PRINTF(_Generic((val), signed char: "%d", unsigned char: "%u", short: "%d", unsigned short: "%u", int: "%d", long: "%ld", long long: "%lld", unsigned: "%u", unsigned long: "%lu", unsigned long long: "%llu", float: "%f", double: "%f", long double: "%Lf", default: _Generic((val - val), ptrdiff_t: "%p", default: "undef")), (val))
#else
/*
...
#if defined(_MSC_VER)
#    define UTEST_SURPRESS_WARNING_BEGIN __pragma(warning(push)) __pragma(warning(disable : 4127)) __pragma(warning(disable : 4571)) __pragma(warning(disable : 4130))
...
#if defined(__clang__)
#    define UTEST_STRNCMP(x, y, size) _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wdisabled-macro-expansion\"") strncmp(x, y, size) _Pragma("clang diagnostic pop")
#else
...
static UTEST_INLINE char *utest_strncpy_gcc(char *const dst, const char *const src, const size_t size) {
#    pragma GCC diagnostic push
...
#    define UTEST_COND(x, y, cond, msg, is_assert)                                                                                                                                                                                                               \
        UTEST_SURPRESS_WARNING_BEGIN do {                                                                                                                                                                                                                        \
            _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wlanguage-extension-token\"") _Pragma("clang diagnostic ignored \"-Wc++98-compat-pedantic\"") _Pragma("clang diagnostic ignored \"-Wfloat-equal\"") UTEST_AUTO(x) xEval = (x); \
...
#    define EXPECT_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message) UTEST_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message, "", 0)
#    define EXPECT_EXCEPTION_WITH_MESSAGE_MSG(x, exception_type, exception_message, msg) UTEST_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message, msg, 0)
#    define ASSERT_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message) UTEST_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message, "", 1)
#    define ASSERT_EXCEPTION_WITH_MESSAGE_MSG(x, exception_type, exception_message, msg) UTEST_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message, msg, 1)
#endif

#if defined(__clang__)
#    if __has_warning("-Wunsafe-buffer-usage")
#        define UTEST_SURPRESS_WARNINGS_BEGIN _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wunsafe-buffer-usage\"")
...
#    define UTEST_SURPRESS_WARNINGS_BEGIN _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wclass-memaccess\"")
...
#define UTEST(SET, NAME)                                                                                                                                                                                \
    ...
    UTEST_INITIALIZER(utest_register_##SET##_##NAME) {                                                                                                                                                  \
        ...
        utest_state.tests        = UTEST_PTR_CAST(struct utest_test_state_s *, utest_realloc(UTEST_PTR_CAST(void *, utest_state.tests), sizeof(struct utest_test_state_s) * utest_state.tests_length)); \
...
#define UTEST_I_TEARDOWN(FIXTURE) static void utest_i_teardown_##FIXTURE(int *utest_result, struct FIXTURE *utest_fixture, size_t utest_index)
...
int                     utest_main(int argc, const char *const argv[]) {
    ...
    for (index = 1; index < UTEST_CAST(size_t, argc); index++) {
        ...
        if (0 == UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
            ...
            seed         = UTEST_CAST(utest_uint32_t, strtoul(argv[index] + strlen(random_order_with_seed_str), UTEST_NULL, 10));
            random_order = 1;
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
        ...
        if (UTEST_TEST_FAILURE == result) {
            ...
            failed_testcases                   = UTEST_PTR_CAST(size_t *, utest_realloc(UTEST_PTR_CAST(void *, failed_testcases), sizeof(size_t) * failed_testcases_length));
            ...
            skipped_testcases                   = UTEST_PTR_CAST(size_t *, utest_realloc(UTEST_PTR_CAST(void *, skipped_testcases), sizeof(size_t) * skipped_testcases_length));
...

        {
            ...
            if (UTEST_TEST_FAILURE == result) {
                printf("%s[  FAILED  ]%s %s (%" UTEST_PRId64 "%s)\n", colours[RED], colours[RESET], utest_state.tests[index].name, time, units[unit_index]);
            } else if (UTEST_TEST_SKIPPED == result) {
                printf("%s[  SKIPPED ]%s %s (%" UTEST_PRId64 "%s)\n", colours[YELLOW], colours[RESET], utest_state.tests[index].name, time, units[unit_index]);
            } else {
                printf("%s[       OK ]%s %s (%" UTEST_PRId64 "%s)\n", colours[GREEN], colours[RESET], utest_state.tests[index].name, time, units[unit_index]);
            }
        }
    }
    ...
    if (0 != skipped) {
        ...
        for (index = 0; index < skipped_testcases_length; index++) {
            printf("%s[  SKIPPED ]%s %s\n", colours[YELLOW], colours[RESET], utest_state.tests[skipped_testcases[index]].name);
        }
    }
